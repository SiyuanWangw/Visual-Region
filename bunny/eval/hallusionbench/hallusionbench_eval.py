...
from bunny.constants import IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
class CustomDataset(Dataset):
    def __init__(self, args, questions, tokenizer, image_processor, image_processor_aux ,model_config):
    ...
    def __getitem__(self, index):
        ...
        image_processor_aux = self.image_processor_aux
        ...
        if line["image"] is None:
            ...
            if self.image_processor_aux is not None:
                image_tensor_aux = process_images([image], image_processor_aux, model_config)
            else:
                image_tensor_aux = None
        input_ids = tokenizer_image_token(prompt, self.tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt').unsqueeze(0).cuda()

        return input_ids, image_tensor, image_tensor_aux ,image_size, prompt
...
def eval_model(args):
    ...
    tokenizer, model, image_processor, image_processor_aux ,context_len = load_pretrained_model(model_path, args.model_base, model_name,args.model_type)

    questions = load_dataset("lmms-lab/HallusionBench", split="image")
    ...
    dataset = CustomDataset(args, questions, tokenizer, image_processor, image_processor_aux ,model.config)
    ...
    for line in tqdm(questions, total=len(questions)):
        ...
        input_ids, image_tensor, image_tensor_aux ,image_sizes, prompt = dataset[ind]
        ...
        with torch.inference_mode():
            output_ids = model.generate(
                ...
                images_aux=image_tensor_aux.to(dtype=model.dtype, device='cuda', non_blocking=True) if image_tensor_aux is not None else  None ,
