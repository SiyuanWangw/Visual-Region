...
class BunnyMetaForCausalLM(ABC):
    ...
    def dual_encode_images(self, images,images_aux):
        image_features = self.get_model().get_vision_tower()(images, return_cls_token=False)
        if images_aux is not None:
            image_features_aux = self.get_model().get_vision_tower_aux()(images_aux, return_cls_token=False)
        else:
            ...
            image_features_aux = self.get_model().get_vision_tower_aux()(images_vae, return_cls_token=False).to(image_features.device).to(image_features.dtype)
        image_features = torch.cat((image_features,image_features_aux), -1)
        image_features = self.get_model().mm_projector(image_features)
        return image_features

    def prepare_inputs_labels_for_multimodal(
        self, input_ids,token_type_ids,position_ids, attention_mask, past_key_values, labels,
        images, images_aux=None,image_sizes=None, cache_position=None,
    ):
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            ...
            return input_ids, token_type_ids ,position_ids, attention_mask, past_key_values, None, labels
        ...
        if vision_tower_aux is not None:
            if type(images) is list or images.ndim == 5:
                ...
                if type(images_aux) is list:
                    images_aux = [x.unsqueeze(0) if x.ndim == 3 else x for x in images_aux]
                concat_images = torch.cat([image for image in images], dim=0)
                concat_images_aux = torch.cat([image_aux for image_aux in images_aux], dim=0)
                image_features = self.dual_encode_images(concat_images,concat_images_aux)
                ...
                image_features = self.dual_encode_images(images,images_aux)
        ...
        return None, token_type_ids ,position_ids, attention_mask, past_key_values, new_input_embeds, new_labels