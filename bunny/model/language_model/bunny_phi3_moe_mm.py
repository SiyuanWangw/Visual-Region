...
from .phi3_moe_mm import Phi3Model, Phi3Config, Phi3ForCausalLM
...
class BunnyMMPhi3MoEConfig(Phi3Config):
    model_type = "bunny-mm-phi3-moe"

    def __init__(self,
                 num_experts_per_tok=2,
                 num_vis_experts = 4,
                 num_lan_experts = 4,
                 vis_router_aux_loss_coef=0.02,
                 lan_router_aux_loss_coef=0.02,
                 output_vis_router_logits = False,
                 output_lan_router_logits = False,
                 **kwargs):
        self.num_experts_per_tok=num_experts_per_tok
        self.num_vis_experts =  num_vis_experts
        self.vis_router_aux_loss_coef = vis_router_aux_loss_coef
        self.output_vis_router_logits = output_vis_router_logits
        self.num_lan_experts =  num_lan_experts
...
class BunnyMMPhi3MoEModel(BunnyMetaModel, Phi3Model):
    config_class = BunnyMMPhi3MoEConfig

    def __init__(self, config: Phi3Config):
        super(BunnyMMPhi3MoEModel, self).__init__(config)


class BunnyMMPhi3MoEForCausalLM(Phi3ForCausalLM, BunnyMetaForCausalLM):
    config_class = BunnyMMPhi3MoEConfig

    def __init__(self, config):
        super(Phi3ForCausalLM, self).__init__(config)
        self.model = BunnyMMPhi3MoEModel(config)
...
AutoConfig.register("bunny-mm-phi3-moe", BunnyMMPhi3MoEConfig)
AutoModelForCausalLM.register(BunnyMMPhi3MoEConfig, BunnyMMPhi3MoEForCausalLM)
